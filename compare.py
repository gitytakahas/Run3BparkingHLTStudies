...
filedict = {'signal': {'file':TFile('out_mc_gabagaba.root'), 'sel':'hlt_dr < 0.2 && hlt_dr > 0. && l1_dr < 0.2 && l1_dr > 0.'},
            'data': {'file':TFile('out_data_gabagaba.root'), 'sel':'isgjson==1'}
}
...
for vkey, ivar in vardict.items():
    ...
    for stype in ['signal', 'data']:

        hist = sproducer(stype, filedict[stype]['file'], vkey, ivar, filedict[stype]['sel'])
